      ...
      <div className="profile-content">
        <div className="profile-photo-section">
          {profileData.profileImage ? (
            ...
            (() => {
              const base = (profileData.name || profileData.email || "User").trim();
              const initials = base
                .split(/\s+/)
                .map((p) => p[0])
                .slice(0, 2)
                .join("")
                .toUpperCase();
              // Simple deterministic background pick
              const palettes = [
                "linear-gradient(135deg, #A5B4FB, #B8FF9F)",
                "linear-gradient(135deg, #FFE500, #FF9F9F)",
                "linear-gradient(135deg, #79F7FF, #A8A6FF)",
                "linear-gradient(135deg, #B8FF9F, #FFF066)",
              ];
              let hash = 0;
              for (let i = 0; i < base.length; i++) hash = (hash + base.charCodeAt(i)) % palettes.length;
              const bg = palettes[hash];
              return (
                <div className="profile-avatar-fallback" style={{ background: bg }} aria-label="User initials">
                  {initials}
                </div>
              );
            })()
          )}
          {editing && <input type="file" accept="image/*" onChange={handlePhotoChange} className="photo-input" />}
        </div>
            ...
            <div className="profile-display">
              ...
              <div className="profile-section">
                <div className="pets-section-header">
                  ...
                  {!showAddPet && (
                    <button className="add-pet-button" onClick={() => setShowAddPet(true)}>
                      Add Pet
                    </button>
                  )}
                </div>
                  ...
                  <div className="pet-form-container">
                    ...
                    <form onSubmit={handlePetSubmit} className="pet-form">
                      <div className="form-row">
                        ...
                        <div className="form-group">
                          <label>Species</label>
                          <select name="species" value={petFormData.species} onChange={handlePetInputChange}>
                      ...
                      <div className="form-row">
                        ...
                        <div className="form-group">
                          <label>Gender</label>
                          <select name="gender" value={petFormData.gender} onChange={handlePetInputChange}>
                      ...
                      <div className="form-group">
                        ...
                        <input type="file" accept="image/*" onChange={handlePetPhotoChange} />
                        {petFormData.profileImage && (
                          <img src={petFormData.profileImage} alt="Pet preview" className="photo-preview" />
                        )}
                      </div>
                        ...
                        <div className="pet-header">
                          ...
                          <div className="pet-basic-info">
                            ...
                            <p className="pet-details">
                              {pet.gender} • {calculateAge(pet.birthDate)} old
                            </p>
                          </div>
                        </div>

                        {pet.birthDate && (
                          <p className="pet-birth">
                            <strong>Born:</strong> {formatDate(pet.birthDate)}
                          </p>
                        )}
              ...
              <div className="profile-section cards-section">
                ...
                <button className="view-cards-button" onClick={() => navigate("/dashboard")}>
